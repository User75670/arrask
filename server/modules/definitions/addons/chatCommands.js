...
class Commands {
	...
	checkPerms(command, socket) {
		...
		if (command.perms & perm.token && permissions.class != 'realdeveloper') {
			command.status('fail').send('You do not have permission to use this command, you must have real developer');
			return false;
		}
		return true;
	}
...
module.exports = ({ Config, Events }) => {
	commands.add({
		...
		execute: ({ command, socket, args }) => {
			...
			if (arg == 'list') {
				command.send(['Available commands:', ...commands.all.filter(c => socket.permissions ? (c.perms == 1 && socket.permissions.class == 'realdeveloper') : c.perms == 0 ).map(c => c.cmdName)]);
			} else {
	...
	commands.add('cap', [1, 2], { doc: 'Set skill cap for the selected entity\nDoc: /cap CAP [WHO]' }, ({ args, body, command }) => {
  	...
  	commands.add('max', [1, 2], { doc: 'Set skill max for the selected entity\nDoc: /max MAX [WHO]' }, ({ args, body, command }) => {
		const max = args.getNumber(0, 0);
		const entity = args.getEntity(1, body);
		if (!entity) return;
		let skills = Array(10).fill(max);
        entity.skill.setCaps(skills);
        entity.skill.set(skills);
		command.send(`Changed ${entity.name}'s skill max`);
	});
