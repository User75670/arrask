...
const gameloop = () => {
    ...
    //   if (
    //     (entities.length > 300 && !e.isPlayer && e.class !== 'rock'  ) ||
    //     (entities.length > 300 && !e.isPlayer && e.class !== 'stone' ) ||
    //     (entities.length > 300 && !e.isPlayer && e.class !== 'gravel') ||
    //     (entities.length > 300 && !e.isPlayer && e.class !== 'wall'  )
    //   ) {
    //     e.kill();
    //   }
    // }
...
let maintainloop = () => {
    ...
    if (!naturallySpawnedBosses.length && bossTimer++ > c.BOSS_SPAWN_COOLDOWN) {
        ...
        setSyncedTimeout(() => {
            ...
            for (let i = 0; i < amount; i++) {
                ...
                boss.define({CONTROLLERS: ['nearestDifferentMaster', 'minion', 'canRepel']});
                if (name) {
                    boss.name = name;
                }
                boss.isBoss = true;
    ...
    if (!global.arenaClosed && bots.length < c.BOTS) {
        ...
        o.color = c.RANDOM_COLORS ? Math.floor(Math.random() * 20) : team ? getTeamColor(team) : 12;
        if (team) o.team = team;
        bots.push(o);
