...
function incoming(message, socket) {
  const cl = socket.permissions?.class ?? "invalid token";
  // Only accept binary
  ...
  switch (m.shift()) {
    ...
    case "smallerTank":
      ...
      if (
        player.body != null &&
        (allowed_perms.shrink.has(cl)) &&
        player.body.SIZE >= minSize
      ) {
    ...
    case "biggerTank":
      ...
      if (
        player.body != null &&
        (allowed_perms.grow.has(cl)) &&
        player.body.SIZE <= maxSize
      ) {
    ...
    case "smallerFOV":
      if (
        player.body != null &&
        (allowed_perms.shrinkFOV.has(cl))
      ) {
        player.body.FOV *= 4 / 5;
      }
      break;
    ...
    case "spectatorKey":
      if (player.body != null &&
        (allowed_perms.spectator.has(cl))
      ) {
    ...
    case "heal":
      {
        // Heal what your mouse is over
        if (player.body != null && 
          (
            allowed_perms.heal.has(cl)
          )
        ) {
          entities.forEach((o) => {
            if (
      ...
      if (player.body != null &&
        ...
        entities.forEach((o) => {
          if (
            ...
            o.sendMessage("You have been kicked by " + player.body.name);
            o.socket.kick('Kick command was requested on ' + o.body.name);
          }
        });
      }
      break;
